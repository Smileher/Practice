...
// 递归进行赫夫曼编码
void HuffmanCode(HuffmanNode *hufmTree, int depth) // depth是赫夫曼树的深度
{
    static int code[10];
    if (hufmTree)
    {
        if (hufmTree->lchild == NULL && hufmTree->rchild == NULL)
        {
            printf("id为%d权值为%d的叶子结点的赫夫曼编码为 ", hufmTree->id, hufmTree->weight);
            int i;
            for (i = 0; i < depth; ++i)
            {
                printf("%d", code[i]);
...
//原文：https://blog.csdn.net/F__shigang/article/details/65442550